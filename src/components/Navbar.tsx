...
export const Navbar = () => {
  ...
  return (
    <nav className="fixed top-6 left-0 right-0 z-50 px-6 pointer-events-none">
      <div className={`max-w-6xl mx-auto glass rounded-full border border-white/10 px-4 py-2 flex justify-between items-center transition-all duration-500 pointer-events-auto ${isScrolled ? 'shadow-[0_10px_30px_rgba(0,0,0,0.5)] scale-95' : 'scale-100'}`}>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:block">
            <a href="#aviso" className="px-6 py-2.5 rounded-full bg-white text-black font-bold text-sm hover:bg-cyan-primary transition-colors">
              Conhecer agora
            </a>
          </div>

          <button className="md:hidden text-white pr-2" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="absolute top-full left-6 right-6 mt-4 glass rounded-[2rem] border border-white/10 p-8 flex flex-col gap-6 md:hidden pointer-events-auto shadow-2xl text-white"
          >
            ...
            <a href="#aviso" className="w-full py-4 rounded-2xl bg-cyan-primary text-black font-bold text-lg text-center" onClick={() => setIsMobileMenuOpen(false)}>
              Conhecer agora
            </a>
          </motion.div>
        )}
